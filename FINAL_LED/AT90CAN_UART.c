...
#define UART1_TX_Buffer_Size 102
circular_buffer UART1_RX_Buffer; 
#define UART1_RX_Buffer_Size 102
...
void USART1_Init( unsigned long baud) {
	...
	UCSR1C	= (0<<UMSEL1) | (0<<UPM1) | (1<<USBS1) | (1<<UCSZ10) | (1<<UCSZ11);
...
ISR(USART1_RX_vect) {
	char received = UDR1; // might be able to eliminate this line. 
	cb_push_back(&UART1_RX_Buffer, &received);
}

ISR(USART0_RX_vect) {
	char received = UDR0; // might be able to eliminate this line.
	cb_push_back(&UART0_RX_Buffer, &received);
}